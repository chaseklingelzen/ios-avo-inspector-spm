...
it(@"inititalizes with app version", ^{
    ...
    expect(appVersion).to.equal(@"13.1");
});
...
it(@"inititalizes with lib version", ^{
    ...
    NSDictionary<NSString *, id> *infoDictionary = [[NSBundle bundleForClass:[sut class]] infoDictionary];
    
    NSString * libVersionInPlist = infoDictionary[@"CFBundleShortVersionString"];
